...
private htmlItemsFromTextContent(
  ...
  const initialMappedItems: EnhancedTextItem[] = rawItems.map((item, index) => {
    ...
    for (const rawName of candidateNames) {
      ...
      exactMatchFound = this.pdfFonts?.find((f: any) => 
    ...
    // Tier 2: CRITICAL FALLBACK FOR INTERNALS (e.g. mapping "g_d0_f1")
    if (!exactMatchFound && this.pdfFonts && this.pdfFonts.length > 0) {
      const hasBoldKeyword = /bold|black|heavy|semibold|demibold|bd|w00/i.test(candidateNames.join(' ')) || /bold|black/i.test(fontKey);
      const hasItalicKeyword = /italic|oblique|it/i.test(candidateNames.join(' ')) || /italic|oblique/i.test(fontKey);
      
      // Look for the specific style variant profile inside your real array
      exactMatchFound = this.pdfFonts.find((f: any) => f.bold === hasBoldKeyword && f.italic === hasItalicKeyword) || 
                        this.pdfFonts.find((f: any) => f.bold === hasBoldKeyword) || 
                        this.pdfFonts[0];
    }
    ...
    } else if (this.pdfFonts && this.pdfFonts.length > 0) {
      trueFontFamilyName = this.pdfFonts[0].baseFont;
      isFontBold = this.pdfFonts[0].bold;
      isFontItalic = this.pdfFonts[0].italic;
    }

    // Standardise rendering fallbacks cleanly
    if (/times/i.test(trueFontFamilyName)) trueFontFamilyName = 'Times New Roman';
    if (/cambria/i.test(trueFontFamilyName)) trueFontFamilyName = 'Cambria';
    if (/calibri/i.test(trueFontFamilyName)) trueFontFamilyName = 'Calibri';
    if (/helvetica/i.test(trueFontFamilyName)) trueFontFamilyName = 'Helvetica';

    // Build the DOM font stacks safely
    const isSerifStyle = /serif|times|cambria|georgia|garamond/i.test(trueFontFamilyName);
                        
    const fallbackStack = isSerifStyle 
      ? `"${trueFontFamilyName}", Georgia, serif` 
      : `"${trueFontFamilyName}", Arial, sans-serif`;

    const textStr = (item.text || '').replace(/\s+/g, ' ').trim();

    // Trigger your service loader to mount the style onto the web layout view context
    this.fontService.loadFontToDOM(trueFontFamilyName);
...
console.log('Extracted PDF font definitions:', this.pdfFonts);
