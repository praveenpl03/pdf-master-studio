...
if (startIdx < 0) { console.log('ERROR: start not found'); process.exit(1); }

// Find the closing brace - it's the line that starts with 2 spaces and } and is followed by blank line + 2 spaces + private
const afterStart = c.substring(startIdx + startMarker.length);
// Match the closing } that ends the function (at indentation level 2)
const endMatch = afterStart.match(/\n  \}/);
if (!endMatch) { console.log('ERROR: end not found'); process.exit(1); }

const endIdx = startIdx + startMarker.length + endMatch.index + 3; // +3 for \n  }
...
console.log('Removed old colorScore (now dead code):', c.includes('colorScore'));
